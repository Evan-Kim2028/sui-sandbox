...
//!
//! End-to-end flow:
//! 1) Pull latest Walrus checkpoints
...
const DEFAULT_LATEST: u64 = 10;
const DEFAULT_TOP_PACKAGES: usize = 8;
const DEFAULT_MAX_PTBS: usize = 20;
...
fn run(args: Args) -> Result<()> {
    ...
    println!("=== Walrus PTB Universe Example ===");
    ...
    let start_cp = latest_cp.saturating_sub(args.latest.saturating_sub(1));
    ...
    let universe = analyze_universe(&loaded, start_cp, latest_cp);
    let summary = universe_summary(&universe, loaded.len());
    ...
    println!("\n=== Completed ===");
    println!("Checkpoints analyzed: {}", loaded.len());
...
fn analyze_universe(
    ...
    for (checkpoint_num, checkpoint_data) in checkpoints {
        for tx in &checkpoint_data.transactions {
            ...
            if let Some(classification) = classify_checkpoint_tx(tx, *checkpoint_num) {
...
fn write_output_readme(
    ...
    let content = format!(
        "# Walrus PTB Universe Output\n\n\
Generated by `cargo run --example walrus_ptb_universe -- --latest {latest} --top-packages {top_packages} --max-ptbs {max_ptbs}`\n\n\
## Window\n\

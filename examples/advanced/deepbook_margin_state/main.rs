...
//! ```
//! SUI_GRPC_ENDPOINT=https://archive.mainnet.sui.io:443
//! SUI_GRPC_API_KEY=your-api-key-here  # optional but recommended
//! ```
//! If `SUI_GRPC_ENDPOINT` is unset, historical mode defaults to an archival-friendly
//! endpoint automatically.
//! If archive replay returns missing-runtime-object aborts, override with:
...
fn main() -> Result<()> {
    ...
    let provider = rt.block_on(common::create_mainnet_provider(checkpoint.is_some()))?;
    println!(
        "  ✓ Connected to mainnet via HistoricalStateProvider ({})\n",
        provider.grpc_endpoint()
    );
    ...
    // Key package roots + type-referenced package roots. This automatically includes
    // package IDs referenced only via type parameters (for example USDC in
    // MarginManager<SUI, USDC>) so users don't need protocol-specific hardcoding.
    ...
    // Reuse the exact endpoint from the initialized provider.
    ...
    if checkpoint.is_some() {
        ...
        let child_fetcher = create_child_fetcher(child_grpc, historical_versions.clone(), None);
    ...
    println!(
        "  Detected {} package upgrades:",
        package_registration_plan.upgrade_map.len()
    );
    for (original, upgraded) in &package_registration_plan.upgrade_map {
        println!(
            "    {} -> {}",
            original.to_hex_literal(),
            upgraded.to_hex_literal()
    ...
    let registration = common::register_packages_with_linkage_plan(
        &mut env,
        &packages,
        &package_registration_plan,
    );
    for (addr, upgraded) in &package_registration_plan.upgrade_map {
        if packages.contains_key(addr) {
            println!(
                "    Skipping 0x{}... (upgraded to 0x{}...)",
                hex::encode(&addr.as_ref()[..4]),
                hex::encode(&upgraded.as_ref()[..4])
            );
        }
    }
    ...
    println!(
        "  ✓ Loaded {} packages with linkage into environment ({} skipped, {} failed)",
        registration.loaded,
    ...
    // Load objects
    common::load_fetched_objects_into_env(&mut env, &fetched_objects, true)?;
    println!(
    ...
    for (parent_id, name) in &versioned_objects {
        match graphql.fetch_dynamic_fields(parent_id, 10) {
            Ok(fields) => {
                ...
                let mut parent_loaded: HashMap<String, common::ExampleFetchedObjectData> =
                    HashMap::new();
                for field in &fields {
                    if let Some(obj_id) = &field.object_id {
                        if let Some((bcs, type_str, version, _)) =
                            common::fetch_object_data(&rt, grpc, obj_id, None, false)
                        {
                            parent_loaded.insert(obj_id.clone(), (bcs, type_str, version, false));
                            println!(
                                "      ✓ DF {} (v{}, key_type={})",
                                &obj_id[..16],
                                version,
                ...
                let loaded_now =
                    common::load_fetched_objects_into_env(&mut env, &parent_loaded, true)?;
                df_count += loaded_now;
            }
            Err(e) => {
    ...
    if result.success {
        ...
        if let Some(raw_error) = &result.raw_error {
            if !combined_error.is_empty() {
                combined_error.push('\n');
            }
            combined_error.push_str(raw_error);
        }

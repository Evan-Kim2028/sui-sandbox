...
//! ## Limitations
//!
//! - Requires the same bytecode version that produced the OBJECT_JSON
...
// Import from sui-sandbox-core utilities
...
impl JsonToBcsConverter {
        ...
        match move_type {
            MoveType::TypeParameter(idx) => {
                if (*idx as usize) < type_args.len() {
                    ...
                    move_type.clone() // Keep as-is if no substitution available
                }
            }
    ...
    fn json_to_dynamic_value_with_type_args(
        ...
        for field_layout in &layout.fields {
            ...
            // Convert to DynamicValue based on type
    ...
    fn convert_field(
        ...
        match move_type {
            ...
            MoveType::U256 => {
                // U256 is typically stored as a hex string or large decimal
    ...
    fn convert_struct(
        ...
        // Build full type string including type arguments for proper lookup
    ...
    fn convert_uid(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        ...
        // UID is a struct with one field: id: ID
        // ID is a struct with one field: bytes: address
        Ok(DynamicValue::Struct {
    ...
    /// Convert an ID field: { "id": "0x..." } or just "0x..."
    ...
    /// Convert a Balance field: { "value": "123" } or just "123"
    fn convert_balance(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        ...
        // Balance is a struct with one field: value: u64
        Ok(DynamicValue::Struct {
    ...
    /// Convert Option<T>: null → None, value → Some(value)
    fn convert_option(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        // Option is serialized as vector with 0 or 1 elements
        if json.is_null() {
            Ok(DynamicValue::Vector(vec![]))
        } else {
            // For now, store as raw - would need type info to fully convert
            let inner = self.infer_and_convert(json, field_name)?;
            Ok(DynamicValue::Vector(vec![inner]))
        }
    }

    /// Convert String (0x1::string::String): plain string in JSON, but internally { bytes: vector<u8> }
    fn convert_string(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        ...
        // String is a struct with one field: bytes: vector<u8>
    ...
    /// Convert TypeName (0x1::type_name::TypeName): { "name": "..." } where name is a String
    fn convert_type_name(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        let name_json = if let Some(obj) = json.as_object() {
            ...
            // Sometimes TypeName might be serialized directly as a string
            json
        };
    ...
    fn convert_vec_set(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        ...
        // VecSet is a struct with one field: contents: vector<T>
        Ok(DynamicValue::Struct {
    ...
    /// Convert VecMap: { "contents": [{ "key": ..., "value": ... }, ...] }
    fn convert_vec_map(&mut self, json: &JsonValue, field_name: &str) -> Result<DynamicValue> {
        ...
        for (i, entry) in arr.iter().enumerate() {
            ...
            // Entry is a struct with key and value
...
fn parse_json_u256(json: &JsonValue, field_name: &str) -> Result<[u8; 32]> {
    if let Some(s) = json.as_str() {
        ...
        // Try parsing as decimal and converting to bytes
        let n: u128 = s
            .parse()

...
/// # Example
/// ```
/// use sui_move_interface_extractor::benchmark::fetcher::{MockFetcher, FetchedObjectData, Fetcher};
///
...
mod tests {
    ...
    fn test_network_fetcher_from_config() {
        ...
        let disabled_config = crate::benchmark::simulation::FetcherConfig::default();
        assert!(NetworkFetcher::from_config(&disabled_config).is_none());

        // Mainnet config
        let mainnet_config = crate::benchmark::simulation::FetcherConfig::mainnet();
        let mainnet_fetcher = NetworkFetcher::from_config(&mainnet_config).unwrap();
        assert_eq!(mainnet_fetcher.network_name(), "mainnet");

        // Testnet config
        let testnet_config = crate::benchmark::simulation::FetcherConfig::testnet();

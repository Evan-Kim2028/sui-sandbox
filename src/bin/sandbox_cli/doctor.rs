...
use sui_transport::network::infer_network_from_endpoints;
...
impl DoctorCmd {
    pub async fn execute(
        ...
        let network = infer_network_from_endpoints(Some(rpc_url), Some(&graphql_endpoint))
            .unwrap_or("mainnet");
        ...
        checks.push(match check_state_file_permissions(state_file) {
        ...
        let grpc_timeout = Duration::from_secs(self.timeout_secs);
        ...
        checks.push(match grpc_check {
            ...
            Err(_) => fail_check(
                "grpc_reachability",
                "gRPC Reachability",
                format!("timed out after {}s", self.timeout_secs),
        ...
        let gql_client = GraphQLClient::with_timeouts(
            ...
            Duration::from_secs(self.timeout_secs.min(10)),
        );

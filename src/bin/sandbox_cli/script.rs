//! Compatibility script/init commands.
//!
//! `pipeline` is the canonical typed orchestration surface.
//! These commands are retained for backward compatibility with older YAML
//! `run-flow` style automation.
...
#[derive(Parser, Debug)]
#[command(about = "Compatibility scaffold for legacy run-flow YAML templates")]
pub struct InitCmd {
...
#[derive(Parser, Debug)]
#[command(about = "Compatibility runner for legacy YAML scripts (run-flow alias)")]
...
impl InitCmd {
    pub async fn execute(&self) -> Result<()> {
        eprintln!(
            "[compat] `init` is a compatibility command. Prefer `sui-sandbox pipeline init ...`."
        );
        ...
        println!(
            "Or migrate to typed pipeline specs:\n  sui-sandbox pipeline init --template generic --output workflow.quickstart.json"
        );
...
impl RunFlowCmd {
    pub async fn execute(
        ...
        eprintln!(
            "[compat] `script`/`run-flow` is compatibility mode. Prefer `sui-sandbox pipeline run --spec ...`."
        );

...
#   ./scripts/internal/entry_function_practical_fuzzer.sh --latest 10 --max-targets 30
#   ./scripts/internal/entry_function_practical_fuzzer.sh --replay-jobs 4
#   ./scripts/internal/entry_function_practical_fuzzer.sh --phase-mode phased
#   ./scripts/internal/entry_function_practical_fuzzer.sh --phase-mode single
#   ./scripts/internal/entry_function_practical_fuzzer.sh --mutation-budget 5 --mutation-jobs 4
#   ./scripts/internal/entry_function_practical_fuzzer.sh --metadata-lookup --include-public
#   ./scripts/internal/entry_function_practical_fuzzer.sh --heal-mode always
...
Usage:
  ./scripts/internal/entry_function_practical_fuzzer.sh [OPTIONS]
...
if [[ -n "${SUI_SANDBOX_BIN:-}" ]]; then
  BIN="$SUI_SANDBOX_BIN"
elif [[ -x "${PROJECT_ROOT}/target/debug/sui-sandbox" ]]; then
  BIN="${PROJECT_ROOT}/target/debug/sui-sandbox"
elif [[ -x "${PROJECT_ROOT}/target/release/sui-sandbox" ]]; then
  BIN="${PROJECT_ROOT}/target/release/sui-sandbox"
else
  echo "Building sui-sandbox binary..."
  cargo build --bin sui-sandbox --manifest-path "${PROJECT_ROOT}/Cargo.toml" >/dev/null
  BIN="${PROJECT_ROOT}/target/debug/sui-sandbox"
fi

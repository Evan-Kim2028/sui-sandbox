...
The **Local Bytecode Sandbox** (`benchmark-local` command) provides offline type inhabitation testing. See [LOCAL_BYTECODE_SANDBOX.md](guides/LOCAL_BYTECODE_SANDBOX.md) for architecture.
...
This framework provides a verifiable, fair baseline for scoring agents. By using bytecode abilities as the source of truth, we ensure:
...
The framework handles the "Benchmarking Lifecycle":
...
- **[Benchmark Guide](guides/RUNNING_BENCHMARKS.md)** - Walkthrough for running benchmarks.
- **[CLI Reference](reference/CLI_REFERENCE.md)** - Rust CLI commands.
- **[JSON Schema](reference/SCHEMA.md)** - Interface and result schemas.

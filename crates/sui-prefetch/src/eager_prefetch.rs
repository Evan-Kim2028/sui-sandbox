...
//!
//! This module provides two prefetching strategies:
//!
//! 1. **Ground-Truth-First** (recommended): Uses `unchanged_loaded_runtime_objects` as the
...
//!
//! 2. **GraphQL Discovery** (legacy): Discovers dynamic field children via GraphQL, then fetches
...
// =============================================================================
// Legacy GraphQL-First Prefetch (for comparison)
// =============================================================================

/// Result of eager prefetching - contains all data needed for replay.
#[derive(Debug, Default)]
pub struct EagerPrefetchResult {
    /// Objects by ID -> (version, type_string, bcs_bytes)
    pub objects: HashMap<String, (u64, Option<String>, Vec<u8>)>,
    /// Packages by ID -> Vec<(module_name, module_bytes)>
    pub packages: HashMap<String, Vec<(String, Vec<u8>)>>,
    /// Dynamic field children by child_id -> (version, type_string, bcs_bytes)
    pub dynamic_children: HashMap<String, (u64, String, Vec<u8>)>,
    /// Statistics
...
pub struct PrefetchStats {
    pub objects_from_tx_metadata: usize,
    pub objects_discovered_via_graphql: usize,
    pub objects_fetched_via_grpc: usize,
    pub packages_fetched: usize,
    pub dynamic_fields_discovered: usize,
    pub fetch_failures: usize,
}

/// Configuration for eager prefetching.
#[derive(Debug, Clone)]
pub struct EagerPrefetchConfig {
    /// Maximum depth for recursive dynamic field discovery
    pub max_depth: usize,
    /// Maximum number of dynamic fields to fetch per object
    pub max_fields_per_object: usize,
    /// Whether to fetch packages transitively
    pub fetch_transitive_packages: bool,
}

impl Default for EagerPrefetchConfig {
    fn default() -> Self {
        Self {
            max_depth: 6,
            max_fields_per_object: 200,
            fetch_transitive_packages: true,
        }
    }
}

/// Eagerly prefetch all data needed to replay a transaction (legacy GraphQL-first approach).
///
/// This function implements a GraphQL discovery approach:
/// 1. Extracts all object IDs and versions from the transaction metadata
/// 2. Uses GraphQL to discover dynamic field children
/// 3. Fetches all discovered objects at their historical versions via gRPC
///
/// For better accuracy and performance, consider using `ground_truth_prefetch_for_transaction`.
pub fn eager_prefetch_for_transaction(
    graphql: &GraphQLClient,
    grpc: &GrpcClient,
    rt: &tokio::runtime::Runtime,
    tx: &GrpcTransaction,
    config: &EagerPrefetchConfig,
) -> EagerPrefetchResult {
    let mut result = EagerPrefetchResult::default();

    // Phase 1: Collect all known object versions from transaction metadata
    let mut historical_versions: HashMap<String, u64> = HashMap::new();

    for (id, ver) in &tx.unchanged_loaded_runtime_objects {
        historical_versions.insert(normalize_address(id), *ver);
        result.stats.objects_from_tx_metadata += 1;
    }
    ...
    // Phase 2: Discover dynamic field children via GraphQL
    ...
    while let Some((parent_id, depth)) = to_visit.pop() {
        if visited.contains(&parent_id) || depth > config.max_depth {
            continue;
        }
        visited.insert(parent_id.clone());

        let dfs = match graphql.fetch_dynamic_fields(&parent_id, config.max_fields_per_object) {
            Ok(fields) => fields,
            Err(_) => continue,
        };

        for df in dfs {
            result.stats.dynamic_fields_discovered += 1;

            if let Some(child_id) = &df.object_id {
                let normalized = normalize_address(child_id);
                all_child_ids.insert(normalized.clone());
                result.stats.objects_discovered_via_graphql += 1;

                if depth < config.max_depth {
                    to_visit.push((normalized.clone(), depth + 1));
                }

                if let Some(ver) = df.version {
                    historical_versions.entry(normalized).or_insert(ver);
                }
            }
        }
    }

    // Phase 3: Fetch all objects at historical versions via gRPC
    let all_objects_to_fetch: HashSet<String> = historical_versions
        .keys()
        .cloned()
        .chain(all_child_ids.iter().cloned())
        .collect();

    for obj_id in &all_objects_to_fetch {
        let version = historical_versions.get(obj_id).copied();

        let fetch_result = rt.block_on(async { grpc.get_object_at_version(obj_id, version).await });

        match fetch_result {
            Ok(Some(obj)) => {
                result.stats.objects_fetched_via_grpc += 1;

                if all_child_ids.contains(obj_id) {
                    if let (Some(type_str), Some(bcs)) = (obj.type_string.clone(), obj.bcs.clone())
                    {
                        result
                            .dynamic_children
                            .insert(obj_id.clone(), (obj.version, type_str, bcs));
                    }
                } else if let Some(bcs) = obj.bcs.clone() {
                    result
                        .objects
                        .insert(obj_id.clone(), (obj.version, obj.type_string.clone(), bcs));
                }

                if let Some(modules) = obj.package_modules {
                    result.packages.insert(obj_id.clone(), modules);
                    result.stats.packages_fetched += 1;
                }
            }
            Ok(None) => {

...
def test_dataset_flag_with_existing_dataset() -> None:
    ...
    result = subprocess.run(
        [
            ...
            "--samples", "0",  # Don't actually run, just validate
            "--agent", "mock-empty",
            "--out", "/tmp/test.json",
        ],
        capture_output=True,
    ...
    try:
        result = subprocess.run(
            [
                ...
                "--corpus-root", "/tmp/fake_corpus",
                "--dataset", "type_inhabitation_top25",
                "--package-ids-file", str(temp_file),
                "--samples", "0",
                "--agent", "mock-empty",
                "--out", "/tmp/test.json",
            ],
        ...
        assert result.returncode != 0
        assert ("Use only one of --dataset or --subset" in result.stderr or
                "Use only one of --dataset or --subset" in result.stdout or
                "Use only one of --dataset or --package-ids-file" in result.stderr or
                "Use only one of --dataset or --package-ids-file" in result.stdout)
    finally:
...
def test_dataset_flag_with_subset_alias() -> None:
    ...
    assert result.returncode != 0
    assert ("Use only one of --dataset or --subset" in result.stderr or
            "Use only one of --dataset or --subset" in result.stdout)

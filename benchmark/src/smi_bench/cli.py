...
def run_all(args):
    ...
    manifest_argv = [
        "--corpus-root", str(args.corpus_root),
        "--out-ids", str(p2_ids),
        "--out-plan", str(p2_plan),
        "--out-report", str(p2_report),
    ]
    if args.samples > 0:
...
def main():
    ...
    p_all.add_argument("--corpus-root", type=Path, required=True, help="Path to sui-packages/packages/mainnet_most_used")
